...
describe('LocalFilesystemWorkspace', () => {
  ...
  // --- check (language-agnostic syntax checking) ---
  ...
  describe('check', () => {
    ...
    it('returns unavailable for unsupported file types', async () => {
      await fs.writeFile(path.join(root, 'src/data.json'), '{}', 'utf-8');

      const result = await ws.check('src/data.json');
      assert.equal(result.totalFiles, 1);
      assert.equal(result.failed, 1);
      assert.ok(result.results[0].errors?.[0].includes('Unsupported file type'));
    });

    it('detects Swift language from .swift extension', async () => {
      // Write valid Swift code
      await fs.writeFile(path.join(root, 'src/valid.swift'), 'let x = 42\n', 'utf-8');

      const result = await ws.check('src/valid.swift');
      assert.equal(result.totalFiles, 1);
      // swiftc may or may not be available
      if (result.passed === 1) {
        assert.equal(result.results[0].language, 'swift');
        assert.equal(result.results[0].success, true);
      } else {
        // unavailable or error
        assert.ok(result.results[0].errors);
      }
    });

    it('detects TypeScript language from .ts extension', async () => {
      await fs.writeFile(path.join(root, 'src/valid.ts'), 'const x: number = 42;\n', 'utf-8');

      const result = await ws.check('src/valid.ts');
      assert.equal(result.totalFiles, 1);
      if (result.passed === 1) {
        assert.equal(result.results[0].language, 'typescript');
      }
    });

    it('detects Python language from .py extension', async () => {
      await fs.writeFile(path.join(root, 'src/valid.py'), 'x = 42\n', 'utf-8');

      const result = await ws.check('src/valid.py');
      assert.equal(result.totalFiles, 1);
      if (result.passed === 1) {
  ...
  describe('check with multiple files', () => {
    let ws: LocalFilesystemWorkspace;
    let root: string;
    let cleanup: () => Promise<void>;
    ...
    before(async () => {
      ...
      await fs.mkdir(path.join(root, 'src'), { recursive: true });
      await fs.writeFile(path.join(root, 'src/a.json'), '{}', 'utf-8');
      await fs.writeFile(path.join(root, 'src/b.txt'), 'hello', 'utf-8');
    });
    ...
    it('handles mixed supported/unsupported files gracefully', async () => {
      const result = await ws.check(['src/a.json', 'src/b.txt']);
      assert.equal(result.totalFiles, 2);
      // Both should fail (unsupported types), but not throw
    ...
    it('compiles Swift files together to resolve cross-file references', async () => {
      // File A defines a type, File B references it.
      // Individual check of B would fail; multi-file check should succeed.
      await fs.writeFile(
        path.join(root, 'src/MyError.swift'),
        'enum MyError: Error {\n  case notFound\n  case timeout\n}\n',
        'utf-8'
      );
      await fs.writeFile(
        path.join(root, 'src/MyHandler.swift'),
        'func handle(_ error: MyError) {\n  switch error {\n  case .notFound: break\n  case .timeout: break\n  }\n}\n',
        'utf-8'
      );

      // Single-file check of MyHandler should fail (can't find MyError)
      const singleResult = await ws.check('src/MyHandler.swift');
      if (singleResult.results[0]?.language === 'swift') {
        // If swiftc is available, it should fail on the cross-file ref
        assert.equal(singleResult.results[0].success, false, 'Single-file check should fail on cross-file reference');
      }

      // Multi-file check should compile both together and succeed
      const multiResult = await ws.check(['src/MyError.swift', 'src/MyHandler.swift']);
      if (multiResult.results[0]?.language === 'swift') {
        // If swiftc available, both should pass when compiled together
        assert.equal(multiResult.passed, 2, `Expected 2 passed, got ${multiResult.passed}. Errors: ${JSON.stringify(multiResult.results)}`);
